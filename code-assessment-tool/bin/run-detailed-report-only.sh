...
"$PYTHON_BIN" "$REPO_ROOT/app.py" "$INPUT_DIR" \
  ...
  --json-out "$OUTPUT_DIR/${SCAN_NAME}_pii-impact-report_${STAMP}.json" \
  --json-file-reports-out "$OUTPUT_DIR/${SCAN_NAME}_pii-file-reports_${STAMP}.json" \
  --csv-file-reports-out "$OUTPUT_DIR/${SCAN_NAME}_pii-file-reports_${STAMP}.csv" \
