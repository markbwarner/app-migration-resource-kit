# Thales Application Protection Maintenance And Support Estimator
...
- AKS and Kubernetes platform operations
- Transactions per second and bulk processing availability
- Monitoring, alert review, and performance management
- Data volumes and number of reveal operations
- Number of application code changes that impact security
- Complexity of applications and systems
...
These examples also assume an efficient operating model built around rollout best practices such as:

- shared security services or reusable wrappers instead of one-off implementations in every app
- solid technology transfer before steady-state support begins
- clear runbooks, support ownership, and escalation paths
- standardized policy templates, naming conventions, and deployment patterns
- automated monitoring, dashboarding, and alert tuning
- stable CI/CD practices and repeatable release controls
- reasonable test automation for protect, reveal, and regression scenarios

When these practices are in place, the incremental steady-state effort is usually lower because teams are not repeatedly rediscovering the same implementation and support issues across applications.
...
- about `5` to `10` total blended incremental hours per month per application
...
- about `11` to `20` total blended incremental hours per month per application
...
That is why final cluster design should remain provisional until transaction-per-second requirements, workload mix, scaling behavior, and the network or egress profile are better understood.
...
Network and egress profile also matter. In practice, pressure often shows up not just as raw TPS, but as a combination of:

- TPS and concurrency
- payload size and bulk-processing volume
- number of outbound connections to the same destinations
- reveal-heavy analytics behavior
- egress path design through load balancer, NAT gateway, or firewall

High-volume clusters can experience SNAT exhaustion, egress bottlenecks, or noisy-neighbor effects even before compute is exhausted, so network behavior should be reviewed as part of cluster planning.
...
- materially different egress volume, connection density, or bandwidth profile
- different outbound architecture requirements such as firewall-routed egress, NAT gateway sizing, or stricter network controls
